...
자세한 내용은 link:https://docs.netapp.com/us-en/data-services-data-classification/task-health-monitor.html["데이터 분류 상태 모니터링"^]를 참조하십시오.
...
스캔 설정에 대한 자세한 내용은 다음을 참조하세요. link:https://docs.netapp.com/us-en/data-services-data-classification/task-managing-repo-scanning.html#view-the-scan-status-for-your-repositories["저장소에 대한 NetApp Data Classification 검사 설정 변경"^].
...
이번 BlueXP classification 릴리스에는 리소스 활용률을 최적화하는 코드 변경 사항이 포함되어 있습니다.
...
Data Classification은 Map 전용 스캔뿐만 아니라 Map & Classify 스캔의 우선순위를 지정할 수 있도록 지원하여 어떤 스캔을 먼저 완료할지 선택할 수 있게 합니다. Map & Classify 스캔의 우선순위 지정은 스캔 시작 전과 도중 모두 지원됩니다. 진행 중인 스캔의 우선순위를 지정하면 매핑 스캔과 분류 스캔 모두 우선순위가 적용됩니다.
...
Data Classification은 분류 이벤트(스캔이 시작될 때와 종료될 때)를 link:https://docs.netapp.com/us-en/console-setup-admin/task-monitor-cm-operations.html#audit-user-activity-from-the-bluexp-timeline["NetApp Console 감사 페이지"^]로 전송하는 것을 지원합니다.
...
매핑 전용 스캔을 수행할 때 가장 중요한 스캔에 우선순위를 지정할 수 있습니다. 이 기능은 작업 환경이 여러 개 있고 우선순위가 높은 스캔을 먼저 완료하려는 경우에 유용합니다.
...
우선순위 지정은 link:https://docs.netapp.com/us-en/data-services-data-classification/concept-classification.html["맵 전용 스캔"^]로 제한되며 맵 및 분류 스캔에는 사용할 수 없습니다.
...
이번 릴리스에는 사소한 개선 사항이 포함되었습니다.

이 릴리스에서는 다음 옵션도 제거되었습니다.

* 감사 로그 수집을 활성화하는 옵션이 비활성화되었습니다.
* 디렉토리 조사 중에는 디렉토리별 개인 식별 정보(PII) 데이터 수를 계산하는 옵션을 사용할 수 없습니다. link:task-investigate-data.html["귀하의 조직에 저장된 데이터를 조사하세요"]을(를) 참조하십시오.
...
Red Hat Enterprise Linux 버전 8 및 9는 이전에 BlueXP 분류 설치에 필요했던 Docker 엔진을 지원하지 않습니다. BlueXP 분류는 이제 컨테이너 인프라로 Podman 버전 4 이상을 사용하여 RHEL 9.0, 9.1 및 9.2에 설치할 수 있습니다. 환경에서 최신 버전의 RHEL을 사용해야 하는 경우 이제 Podman을 사용할 때 BlueXP 분류(버전 1.26 이상)를 설치할 수 있습니다.

현재 NetApp Data Classification은 RHEL 9.x를 사용할 때 다크 사이트 설치나 분산 스캐닝 환경(마스터 및 원격 스캐너 노드 사용)을 지원하지 않습니다.
...
=== BlueXP 분류 UI에서 사용자 지정 키워드를 추가하세요
...
"액세스 권한이 있는 사용자 수" 필터가 이제 SharePoint Online 리포지토리에 저장된 파일도 지원합니다. 이전에는 CIFS 공유에 있는 파일만 지원되었습니다. SharePoint 그룹 중 Active Directory 기반이 아닌 그룹은 현재 이 필터에 포함되지 않습니다.
