...
---
= NetApp Data Classification을 통해 조직의 개인 데이터에 대한 규정 준수 세부 정보를 확인하세요
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: 파일 수준의 규정 준수 세부 정보는 전체 분류 스캔을 수행한 경우에만 확인할 수 있습니다. 매핑 전용 스캔에서는 파일 수준의 세부 정보가 제공되지 않습니다.
...
데이터 분류는 데이터 내의 특정 단어, 문자열, 패턴(정규식)을 자동으로 식별합니다. link:reference-private-data-categories.html["예를 들어, 신용카드 번호, 주민등록번호, 은행 계좌번호, 비밀번호 등이 있습니다."]데이터 분류는 개별 파일, 디렉토리(공유 및 폴더) 내의 파일, 데이터베이스 테이블에서 이러한 유형의 정보를 식별합니다.
...
+
image:screenshot_compliance_investigation_page.png["조사 결과를 선택한 후 파일에서 발견된 상세 정보의 스크린샷입니다."]

+
image:screenshot_compliance_investigation_page_directory.png["'조사 결과'를 선택한 후 디렉터리에서 발견된 상세 정보의 스크린샷입니다."]
...
. 특정 파일에 대한 세부 정보를 검색, 정렬, 확장하거나 *조사 결과*를 선택하여 마스킹된 정보를 확인하거나 파일 목록을 다운로드하여 데이터를 조사합니다.
