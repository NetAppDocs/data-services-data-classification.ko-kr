...
[role="lead"]
NetApp Data Classification은 NetApp Console에서 제공되는 데이터 거버넌스 서비스로, 온프레미스 및 클라우드 데이터 소스를 스캔하여 데이터를 매핑 및 분류하고 개인 정보를 식별합니다. 이를 통해 보안 및 규정 준수 위험을 줄이고 스토리지 비용을 절감하며 데이터 마이그레이션 프로젝트를 지원할 수 있습니다.


IMPORTANT: 버전 1.31부터 데이터 분류 기능은 NetApp Console의 핵심 기능으로 제공됩니다. 추가 비용은 없습니다. 데이터 분류 라이선스나 구독도 필요하지 않습니다. + 기존 버전 1.30 이하를 사용 중인 경우 구독 만료일까지 해당 버전을 계속 사용할 수 있습니다.
...
NetApp Console은 엔터프라이즈 규모의 온프레미스 및 클라우드 환경 전반에서 NetApp 스토리지 및 데이터 서비스의 중앙 집중식 관리를 제공합니다. NetApp 데이터 서비스에 액세스하고 사용하려면 Console이 필요합니다. 관리 인터페이스로서 하나의 인터페이스에서 많은 스토리지 리소스를 관리할 수 있도록 지원합니다. Console 관리자는 엔터프라이즈 내의 모든 시스템에 대한 스토리지 및 서비스 액세스를 제어할 수 있습니다.
...
.저장 공간 사용량 최적화
Data Classification은 스토리지 총 소유 비용(TCO)을 줄이는 데 도움이 되는 툴을 제공합니다. Data Classification을 사용하여 다음을 수행할 수 있습니다.
...
데이터 전송 비용은 설정에 따라 다릅니다. 데이터 분류 인스턴스와 데이터 소스가 동일한 가용 영역 및 지역에 있는 경우 데이터 전송 비용이 발생하지 않습니다. 하지만 Cloud Volumes ONTAP 시스템 등 데이터 소스가 _다른_ 가용 영역 또는 지역에 있는 경우 클라우드 공급자로부터 데이터 전송 비용이 청구됩니다. 자세한 내용은 다음 링크를 참조하십시오.
...
* AWS에서 Data Classification은 500 GiB GP2 디스크가 있는 https://aws.amazon.com/ec2/instance-types/m6i/["m6i.4xlarge 인스턴스"^]에서 실행됩니다. 운영 체제 이미지는 Amazon Linux 2입니다.
...
* **매핑 전용 스캔**은 데이터에 대한 개략적인 개요만 제공하며 선택한 데이터 소스에서 수행됩니다. 매핑 전용 스캔은 내부 데이터를 보기 위해 파일에 액세스하지 않으므로 전체 스캔보다 시간이 덜 걸립니다. 처음에 이 방법을 사용하여 조사 영역을 식별한 다음 해당 영역에 대해 전체 스캔을 수행할 수 있습니다.
* **전체 스캔**은 데이터에 대한 심층적인 스캔을 제공합니다. 전체 스캔에는 지도 전용 스캔과 파일 내 데이터 분류가 포함됩니다.


맵 전용 스캔과 전체 스캔의 차이점에 대한 분석은 link:task-scanning-overview.html["매핑 스캔과 분류 스캔의 차이점은 무엇인가요?"]을 참조하십시오.
