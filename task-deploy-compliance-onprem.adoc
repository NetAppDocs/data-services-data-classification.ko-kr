...
summary: 네트워크의 Linux 호스트나 인터넷 접속이 가능한 클라우드의 Linux 호스트에 NetApp 데이터 분류를 배포하려면 네트워크나 클라우드에 Linux 호스트를 수동으로 배포해야 합니다. 
---
= 인터넷 접속이 가능한 호스트에 NetApp 데이터 분류 설치
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
네트워크의 Linux 호스트나 인터넷 접속이 가능한 클라우드의 Linux 호스트에 NetApp 데이터 분류를 배포하려면 네트워크나 클라우드에 Linux 호스트를 수동으로 배포해야 합니다.
...
아직 콘솔 에이전트가 없는 경우 https://docs.netapp.com/us-en/bluexp-setup-admin/task-quick-start-connector-on-prem.html["온프레미스에 콘솔 에이전트 배포"^] 네트워크의 Linux 호스트나 클라우드의 Linux 호스트에서.
...
데이터 분류를 설치하고 사용하려면 콘솔 에이전트가 필요합니다.  대부분의 경우 데이터 분류를 활성화하기 전에 콘솔 에이전트를 설정했을 가능성이 높습니다. https://docs.netapp.com/us-en/bluexp-setup-admin/concept-connectors.html["콘솔 기능에는 콘솔 에이전트가 필요합니다."] 하지만 지금 당장 설정해야 하는 경우도 있습니다.
...
또한 다음을 수행할 수도 있습니다. https://docs.netapp.com/us-en/bluexp-setup-admin/task-quick-start-connector-on-prem.html["온프레미스에 콘솔 에이전트 배포"^] 네트워크의 Linux 호스트나 클라우드의 Linux 호스트에서.  온프레미스에 데이터 분류를 설치하려는 일부 사용자는 콘솔 에이전트도 온프레미스에 설치하기로 선택할 수도 있습니다.
...
....
+
추가 데이터 분류 호스트를 스캐너 노드로 사용할 계획이라면 지금 기본 시스템에 다음 규칙을 추가하세요.
...
| \ https://api.bluexp.netapp.com | NetApp 계정을 포함한 콘솔과의 통신. 
...
NOTE: 현재 데이터 분류 기능은 온프레미스에 소프트웨어가 설치된 경우 S3 버킷, Azure NetApp Files 또는 FSx for ONTAP 검색할 수 없습니다.  이러한 경우 클라우드에 별도의 콘솔 에이전트와 데이터 분류 인스턴스를 배포해야 합니다. https://docs.netapp.com/us-en/bluexp-setup-admin/concept-connectors.html["커넥터 간 전환"^] 다양한 데이터 소스에 대해.
...
. 예를 들어, 호스트 컴퓨터에서 설치 프로그램 파일의 압축을 풉니다.
+
[source, cli]
----
...
. 클라우드에서 준비한 인스턴스에 데이터 분류를 설치하는지, 아니면 사내에서 준비한 인스턴스에 데이터 분류를 설치하는지에 따라 적절한 *배포* 버튼을 선택하여 데이터 분류 설치를 시작합니다.
+
image:screenshot_cloud_compliance_deploy_onprem.png["클라우드 또는 사내 머신에 데이터 분류를 배포하기 위한 버튼을 선택하는 스크린샷입니다."]
