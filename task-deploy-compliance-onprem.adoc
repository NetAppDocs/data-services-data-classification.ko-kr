...
summary: 네트워크의 Linux 호스트 또는 인터넷에 연결된 클라우드의 Linux 호스트에 NetApp Data Classification을 설치하려면 네트워크 또는 클라우드에 Linux 호스트를 수동으로 배포해야 합니다. 
---
= 인터넷에 접속 가능한 호스트에 NetApp Data Classification 설치
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
네트워크의 Linux 호스트 또는 인터넷에 연결된 클라우드의 Linux 호스트에 NetApp Data Classification을 배포하려면 네트워크 또는 클라우드에 Linux 호스트를 수동으로 배포해야 합니다.
...
Data Classification을 설치하고 사용하려면 Console 에이전트가 필요합니다. 대부분의 경우 Data Classification을 활성화하기 전에 Console 에이전트가 이미 설정되어 있을 것입니다.
...
대부분의 경우 데이터 분류는 단일 호스트 시스템에 설치합니다. 이는 콘솔 에이전트가 설치된 VM 호스트와는 다른 호스트입니다. <<일반적인 구성을 위한 단일 호스트 설치,여기에서 해당 단계를 확인하세요>>.
...
.단계
.  https://mysupport.netapp.com/site/products/all/details/cloud-data-sense/downloads-tab/["NetApp 지원 사이트"^]에서 Data Classification 소프트웨어를 다운로드하십시오. *DATASENSE-INSTALLER-<version>.tar.gz*라는 파일을 선택하십시오.
