...
참고로, link:task-deploy-compliance-onprem.html["인터넷 접속이 가능한 Linux 호스트에 데이터 분류 설치"]. 온프레미스에 있는 Data Classification 인스턴스를 사용하여 온프레미스 ONTAP 시스템을 스캔하는 것을 선호하는 경우 이 설치 유형이 좋은 선택일 수 있지만, 필수 사항은 아닙니다. 어떤 설치 방법을 선택하든 소프트웨어의 기능은 완전히 동일합니다.
...
콘솔 에이전트가 아직 없는 경우 클라우드 공급자에서 콘솔 에이전트를 생성하십시오.  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["AWS에서 콘솔 에이전트 만들기"^] 또는  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-azure.html["Azure에서 콘솔 에이전트 만들기"^], 또는  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-google.html["GCP에서 콘솔 에이전트 만들기"^]을 참조하십시오. 대부분의 경우 Data Classification을 활성화하기 전에 콘솔 에이전트가 설정되어 있을 가능성이 높지만 https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["콘솔 기능에는 콘솔 에이전트가 필요합니다."], 지금 설정해야 하는 경우도 있습니다.

특정 클라우드 공급자에 배포된 콘솔 에이전트를 사용해야 하는 경우가 있습니다.
