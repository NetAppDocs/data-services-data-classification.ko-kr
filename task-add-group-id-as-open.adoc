...
summary: 'NFS 파일 공유에서 그룹 ID가 파일이나 폴더에 연결되면 해당 파일이나 폴더에 대한 권한을 정의합니다. 그룹 ID가 "조직에 공개" 권한 수준으로 설정되어 있지 않은 경우, 해당 GID에 해당 권한을 추가하여 해당 GID가 연결된 모든 파일과 폴더가 "조직에 공개"된 것으로 간주되도록 할 수 있습니다.' 
---
