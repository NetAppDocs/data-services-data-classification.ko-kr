...
summary: 웹 UI를 통해 제공되는 NetApp Data Classification 기능은 API를 통해서도 사용할 수 있습니다. 
---
...
[role="lead"]
NetApp 데이터 분류 REST API를 사용하면 웹 UI에서 사용할 수 있는 것과 동일한 기능을 이용할 수 있습니다.
...
** 데이터는 JSON 형식으로 내보내집니다(Splunk 등 타사 애플리케이션으로 파싱 및 전송하기 용이함).


* "AND" 및 "OR" 문을 사용하여 쿼리를 만들고, 정보를 포함하거나 제외하는 등의 작업을 수행합니다.
+
예를 들어, 특정 개인 식별 정보(PII)가 포함되지 않은 파일을 찾을 수 있습니다(사용자 인터페이스에서는 제공되지 않는 기능). 또한 내보내기 작업에서 특정 필드를 제외할 수도 있습니다.
...
Swagger에 액세스하려면 Data Classification 인스턴스의 IP 주소가 필요합니다. 클라우드 배포의 경우 공용 IP 주소를 사용합니다. 그런 다음 이 엔드포인트에 액세스해야 합니다.
...
.API 요청
조사 탭에서 모든 필터를 보려면 시스템에 필요한 모든 관련 필드와 옵션을 가져와야 합니다.
...
----
요청 매개변수의 응답을 사용하여 복사할 파일을 필터링합니다.
