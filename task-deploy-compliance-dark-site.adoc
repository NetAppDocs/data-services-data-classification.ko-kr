...
[role="lead"]
인터넷에 접속할 수 없는 온프레미스 사이트의 Linux 호스트에 NetApp Data Classification을 설치하려면 _프라이빗 모드_를 사용하십시오. 이 배포 방식은 설치 스크립트를 사용하며 데이터 분류 인스턴스를 NetApp Console SaaS 계층에서 분리된 상태로 유지하므로 격리된 환경에서 로컬 데이터 소스를 스캔할 수 있습니다.
