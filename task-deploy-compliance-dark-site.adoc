...
---
= 프라이빗 모드의 Linux 호스트에 NetApp Data Classification 설치
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
_프라이빗 모드_를 사용하여 인터넷에 액세스할 수 없는 온프레미스 사이트의 Linux 호스트에 NetApp Data Classification을 설치합니다(오프라인 설치 또는 다크 사이트라고도 함). 이 배포는 설치 스크립트를 사용하며 Data Classification 인스턴스를 NetApp Console SaaS 계층에서 연결 해제된 상태로 유지하여 격리된 환경에서 로컬 데이터 소스를 스캔할 수 있도록 합니다.
...
데이터 분류 설치 스크립트는 시스템 및 환경이 필수 요구 사항을 충족하는지 확인합니다. 요구 사항을 충족하면 설치가 시작됩니다. 데이터 분류 설치를 실행하기 전에 별도로 요구 사항을 확인하려면 요구 사항만 테스트하는 별도의 소프트웨어 패키지를 다운로드할 수 있습니다. link:task-test-linux-system.html["Linux 호스트가 데이터 분류를 설치할 준비가 되었는지 확인하는 방법을 알아보세요."].
...
개인 모드로 설치된 경우, NetApp Data Classification은 온프레미스 사이트에 로컬인 데이터 소스의 데이터만 스캔할 수 있습니다. 현재 NetApp Data Classification에서 스캔할 수 있는 *로컬* 데이터 소스는 다음과 같습니다.
...
Data Classification이 콘솔 에이전트에 배포된 경우, 콘솔 에이전트를 다른 호스트로 마이그레이션하는 것은 지원되지 않습니다. 다크 사이트의 다른 콘솔 에이전트에서 Classification이 필요한 경우, 새 호스트에 새 콘솔 에이전트와 Data Classification 인스턴스를 배포하십시오.
...
* Data Classification은 전용 호스트에 있어야 합니다. 호스트는 바이러스 백신 등 다른 애플리케이션이나 타사 소프트웨어와 공유할 수 없습니다.
...
** *Azure VM 크기*: "Standard_D16_v5". link:reference-instance-types.html#azure-instance-types["추가 Azure 인스턴스 유형 보기"^].
...
* *Red Hat Subscription Management*: 호스트는 Red Hat Subscription Management에 등록되어 있어야 합니다. 등록되어 있지 않으면 시스템이 설치 중에 필요한 타사 소프트웨어를 업데이트하기 위한 저장소에 액세스할 수 없습니다.
...
NOTE: 데이터 분류 설치 프로그램은 UID 4321과 GID 4321을 예약해 두고 있습니다. 설치를 시작하기 전에 호스트 컴퓨터에서 UID 4321과 GID 4321이 사용 중이 아닌지 확인하십시오. 사용 중인 경우 "잘못된 사용자" 오류 메시지와 함께 설치가 실패합니다.
...
대부분의 경우 데이터 분류는 단일 호스트 시스템에 설치합니다. 이 시스템은 콘솔 에이전트가 설치된 VM 호스트와는 다른 시스템입니다.

image:diagram_deploy_onprem_single_host_no_internet.png["인터넷 연결 없이 온프레미스에 배포된 단일 Data Classification 인스턴스를 사용할 때 스캔할 수 있는 데이터 소스의 위치를 보여주는 다이어그램입니다."]
